...
public abstract  class BaseServiceImpl<T> implements BaseService<T>{
	 ...
	 * 根据参数查询
	 */
	public T listByParms(String params) {
		return this.getBaseDao().listByParms(params);
	}
	...
	public T load1(Serializable id) {
		return this.getBaseDao().load1(id);
	}
     ...
     * @return
     */
    public Pager<T> findByEntity(T entity,Integer pageNum,Integer pageSize){
    	/**
		 * 执行分页
		 */
		if(pageSize==null||pageSize<0) pageSize = 5;
		if(pageNum==null||pageNum<0) pageSize = 1;
		//开启分页
		PageHelper.startPage(pageNum, pageSize);
     ...
     * @return
     */
	public Pager<T> findBySqlRerturnEntity(String sql,Integer pageNum,Integer pageSize) {
		/**
		 * 执行分页
		 */
//		Integer pageSize = SystemContext.getPageSize();//每页的数据个数
//		Integer pageOffset = SystemContext.getPageOffset();//偏移量
//		if(pageOffset==null||pageOffset<0) pageOffset = 0;
		if(pageSize==null||pageSize<0) pageSize = 5;
		if(pageNum==null||pageNum<0) pageNum = 1;
//		String order = SystemContext.getOrder();//列表的排序字段
//		String sort = SystemContext.getSort();//列表的排序方式
//		Integer pageNum = null;//第几页
//		if(pageOffset == 0){
//			pageNum = 1;
//		}else{
//			pageNum = pageOffset/pageSize+1;
